// Shared local/Vercel /api/chat pipeline. Deterministic understanding and
// decision logic remains authoritative for safety; Ollama is the communication
// layer for routine medical turns when available, with deterministic fallback.
// RAG retrieves matching chunks from built-in/admin documents and supplies them
// as reference context; retrieved text never overrides approved guidance.
const fs = require('fs');
...
const { blankContext, updateContext, getRelevantContext, extractWoundLocation } = require('./context.js');
...
const { generateAIResponse } = require('./ai-response.js');
...
function buildSingleKitItemReply(text) {
    const n = String(text || '').toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
    const items = [
        { names:['adhesive bandage','adhesive bandages','band aid','band-aid','bandage'], name:'Adhesive Bandages (Assorted Sizes)', purpose:'cover a small, cleaned cut or scrape after bleeding is controlled', steps:['Clean the minor wound with clean running water.','Make sure bleeding is controlled.','Place the bandage over the wound and replace it if it becomes wet or dirty.'] },
        { names:['sterile gauze','gauze pads','gauze'], name:'Sterile Gauze Pads', purpose:'cover wounds and help control bleeding', steps:['Place clean gauze over the wound.','For active bleeding, apply firm, continuous direct pressure.','If blood soaks through, keep the original gauze in place and add more on top while continuing pressure.'] },
        { names:['medical adhesive tape','medical tape','adhesive tape'], name:'Medical Adhesive Tape', purpose:'secure gauze, dressings, or bandages', steps:['Place the dressing or gauze over the wound first.','Use short pieces of tape to secure the edges.','Do not wrap tape tightly around a limb.'] },
        { names:['cotton swab','cotton swabs'], name:'Cotton Swabs', purpose:'minor cleaning or applying a product to a small surface area', steps:['Use only on a superficial area where appropriate.','Do not insert a cotton swab into the eye, ear, or a deep wound.','Do not use it to dig out embedded objects.'] },
        { names:['burn dressing','non stick burn dressing','non-stick burn dressing'], name:'Burn Dressing (Sterile Non-Stick)', purpose:'protect a minor burn without sticking to injured skin', steps:['Cool the minor burn with clean, cool running water first.','Do not apply ice directly to the skin.','Place the sterile non-stick dressing gently over the cooled burn and secure it without tight pressure.'] },
        { names:['antiseptic solution','antiseptic wipes','antiseptic'], name:'Antiseptic Solution / Wipes', purpose:'clean the skin around a minor wound', steps:['Follow the product label.','Use it on the surrounding skin as directed.','Do not get it in the eyes unless the product is specifically made for eye use.'] },
        { names:['isopropyl alcohol','hand sanitizer','alcohol sanitizer','sanitizer'], name:'Isopropyl Alcohol / Hand Sanitizer', purpose:'hand hygiene before providing first aid', steps:['Use it to clean your hands when soap and water are unavailable.','Allow your hands to dry before providing care.','Do not pour alcohol or hand sanitizer into an open wound, and keep it away from flames.'] },
        { names:['disposable medical gloves','medical gloves','gloves'], name:'Disposable Medical Gloves', purpose:'protect the rescuer and patient from contact with blood or body fluids', steps:['Put the gloves on before contact with blood or body fluids.','Avoid touching clean surfaces unnecessarily while wearing them.','Remove and dispose of them safely after care, then clean your hands.'] },
        { names:['medical face mask','medical face masks','face mask','face masks'], name:'Medical Face Masks', purpose:'basic respiratory infection-control protection', steps:['Place the mask over the nose and mouth and secure it according to its design.','Avoid touching the front while wearing it.','Replace it if wet or soiled and dispose of it appropriately.'] },
        { names:['cpr face shield','cpr barrier','face shield','barrier device'], name:'CPR Face Shield / Barrier Device', purpose:'provide a barrier during CPR and rescue breathing', steps:['Call 911 and begin CPR when indicated.','Place the barrier according to its product instructions before rescue breaths if using conventional CPR and trained to do so.','Follow the dispatcher or trained-responder instructions and continue CPR until help takes over.'] },
        { names:['elastic bandage','ace bandage'], name:'Elastic Bandage (ACE Type)', purpose:'gentle support or compression for a minor sprain or strain', steps:['Wrap the injured area snugly, not tightly.','Check fingers or toes for normal color, warmth, sensation, and comfort.','Loosen it if there is numbness, tingling, increasing pain, or color change.'] },
        { names:['triangular bandage','triangular'], name:'Triangular Bandage', purpose:'make a sling or provide basic support for an injured arm', steps:['Support the injured arm in a comfortable position without forcing movement.','Use the triangular bandage to form a sling according to its design.','Seek medical assessment for severe pain, deformity, or inability to move the area.'] },
        { names:['instant cold pack','instant cold packs','cold pack','cold compress'], name:'Instant Cold Packs / Compresses', purpose:'reduce swelling and discomfort from minor bumps or sprains', steps:['Activate the pack according to the package directions.','Wrap it in a cloth barrier before placing it on skin.','Use it for the time specified on the package and stop if the skin becomes painful, numb, or unusually pale.'] },
        { names:['medical scissors','safety scissors','scissors'], name:'Medical Scissors', purpose:'cut gauze, tape, bandages, or clothing safely near the skin', steps:['Cut only material that needs to be removed or trimmed.','Keep the blades away from the skin when cutting clothing or dressings.','Use extra care around an injured person and never use them to probe a wound.'] },
        { names:['fine tip tweezers','fine-tip tweezers','tweezers'], name:'Fine-Tip Tweezers', purpose:'remove a superficial splinter or superficial foreign material', steps:['Clean the tweezers and the surrounding skin as appropriate.','Grip a clearly visible superficial splinter close to the skin and pull it out along its entry direction.','Do not dig into skin or remove objects embedded in the eye, neck, chest, or another serious wound.'] },
        { names:['digital thermometer','thermometer'], name:'Digital Thermometer', purpose:'check body temperature', steps:['Use it according to the thermometer instructions.','Wait for the reading and record the temperature and time if monitoring illness.','Clean it as directed after use.'] },
        { names:['emergency thermal blanket','thermal blanket','mylar blanket'], name:'Emergency Thermal Blanket (Mylar)', purpose:'help maintain body warmth while waiting for help or after cold exposure', steps:['Wrap the person in a dry layer and use the blanket to retain body heat.','Keep the person dry and monitor their condition.','Do not use it to cover someone who is actively overheating.'] },
        { names:['sterile saline','saline solution','saline'], name:'Sterile Saline Solution', purpose:'rinse the eyes or flush minor wounds', steps:['Use sterile saline intended for wound or eye irrigation.','For an eye, gently flush without rubbing or digging into the eye.','Seek urgent medical help for chemical exposure, an embedded object, severe pain, or vision changes.'] },
    ];
    // Match the longest actual alias first so "elastic bandage" does not
    // fall through to the shorter generic "bandage" alias.
    const candidates = [];
    for (const item of items) {
        for (const alias of item.names) {
            const escaped = alias.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            if (new RegExp(`(^|\\s)${escaped}(?=\\s|$)`, 'i').test(n)) candidates.push({ item, alias });
        }
    }
    candidates.sort((a,b) => b.alias.length - a.alias.length);
    const item = candidates[0]?.item || null;
    if (!item) return null;
    if (!/\b(what|how|purpose|use|uses|used|for|do|does|work|mean|meaning)\b/i.test(n) && !/\b(about|tell me)\b/i.test(n)) return null;
...
const INTENT_TERMS = {
    ...
    medkit_inventory: ['medkit', 'med kit', 'medical kit', 'medical aid kit', 'first aid kit', 'first-aid kit', 'aid kit', 'the kit', 'my kit', 'what do i need', 'supplies', 'equipment']
};
...
const ESCALATION_TERMS = [
    ...
    'not waking up', 'is not responding', 'isn\'t responding', 'aren\'t responding', 'are not responding', 'not responsive', 'hindi tumitigil', 'di tumitigil', 'lumalala',
    'blister got bigger', 'blister is getting bigger', 'blister grew',
    'skin looks really damaged', 'skin looks badly damaged', 'skin is badly damaged',
    'skin is peeling off', 'skin peeled off', 'skin is coming off'
];
...
const ESCALATION_EXPANSIONS = {
    ...
    burn_care: 'worsening burn growing blister damaged skin severe burn deep burn large burn electrical burn chemical burn urgent medical assessment emergency call 911',
...
function buildFocusedFollowupReply(understanding, structured = {}, userText = '') {
    const n = String(userText || '').toLowerCase().replace(/[’‘]/g, "'").replace(/\s+/g, ' ').trim();
    const topics = new Set(understanding?.topics || []);
    const facts = structured?.relevantContext?.facts || {};
    if (topics.has('minor_wound') && /\b(what should i put|what do i put|what can i put|what should i use|what do i use)\b/i.test(n)) {
        const location = facts.wound_location ? ` on your ${facts.wound_location}` : '';
        return `For the cut${location}, gently rinse it with clean running water, then cover it with a clean **adhesive bandage**. ${facts.bleeding_status === 'not bleeding' ? 'Since it is no longer bleeding, you do not need to keep applying pressure.' : 'If it starts bleeding again, use **sterile gauze pads** and apply steady pressure.'}`;
    }
    if (topics.has('minor_wound') && /\b(what should i do|what do i do|what should i do now|what do i do now|what now|what next)\b/i.test(n)) {
        const location = facts.wound_location ? ` on your ${facts.wound_location}` : '';
        if (facts.bleeding_status === 'still bleeding') {
            return `For the cut${location}, keep applying steady direct pressure with **sterile gauze** until the bleeding is controlled. Once it stops, gently rinse the cut and cover it with an **adhesive bandage** if it is small.`;
        }
        return `For the cut${location}, gently rinse it with clean running water and cover it with a clean **adhesive bandage**. Since you already said the bleeding is controlled, you can focus on keeping it clean and protected.`;
    }
    if (topics.has('minor_wound') && /\b(pain|painful|hurt|hurts|sore)\b/i.test(n)) {
        return facts.bleeding_status === 'still bleeding'
            ? 'Some pain can occur with a cut. Since it is still bleeding, keep steady direct pressure with **sterile gauze** until the bleeding is controlled.'
            : 'Some pain can occur with a cut. Keep it clean and protected, and seek medical attention if the pain is severe, worsening, or the wound appears more serious than a minor cut.';
    }
    if (topics.has('burn') && /\b(what should i do|what do i do|what now|what next)\b/i.test(n)) {
        return 'For your hand, cool the burn under clean running water and **do not apply ice directly to the skin**. After cooling, a **sterile non-stick burn dressing** can protect a minor burn.';
    }
    return null;
}

function buildUrgentMedicalFallback(understanding, structured = {}, guidance = null) {
    const topics = new Set(understanding?.topics || []);
    const contextFacts = structured?.relevantContext?.facts || {};

    if (topics.has('burn')) {
        const worsening = /blister|damaged|worsen|worse|peeling|coming off/i.test(
            String(contextFacts.associated_symptoms_or_severity || '') + ' ' +
            String(contextFacts.burn_extent_or_location || '') + ' ' +
            String(structured?.context?.originMessage || '')
        );
        const reason = worsening
            ? 'Because the blister has enlarged or the skin looks significantly damaged, this burn should be assessed promptly by a healthcare professional.'
            : 'Because the burn involves the hand or may be more than a minor burn, it should be assessed promptly by a healthcare professional.';
        return `${reason}\n\n- **Cool the burn with clean, cool running water** if you have not already done so.\n- **Do not apply ice directly to the skin and do not break the blister.**\n- Protect the area with a **sterile non-stick burn dressing** without pressing on damaged skin.\n\nIf the burn is deep, extensive, rapidly worsening, or you have severe pain, loss of sensation, or other serious symptoms, seek emergency help.`;
    }

    if (topics.has('fracture') || topics.has('dislocation')) {
        return 'This injury may be more serious than a minor sprain. **Get prompt medical assessment and avoid putting unnecessary weight or force on the area.**\n\n- Keep the injured area as still and supported as you can.\n- **Do not try to straighten a deformed joint or push a bone back into place.**\n- Get emergency help for severe deformity, numbness or coldness, uncontrolled bleeding, or other emergency signs.';
    }

    if (topics.has('injury') || topics.has('sprain')) {
        const actions = guidance?.actions || ['Stop the activity and rest the injured area.', 'Use a wrapped cold pack for swelling and avoid forcing painful movement.'];
        return 'Because the injury may be more serious or is getting worse, **it should be assessed promptly if the symptoms are significant or continuing to worsen.**\n\n' + actions.map(a => `- ${a}`).join('\n') + '\n\nGet urgent medical help for severe pain, major swelling or deformity, inability to use the limb, numbness, or circulation problems.';
    }

    if (guidance) {
        return `Because this situation may be more serious or is getting worse, **seek prompt medical assessment.**\n\n${guidance.actions.map(a => `- ${a}`).join('\n')}\n\n${guidance.summary}`;
    }
    return null;
}
...
function buildFormattedMedicalReply(understanding, structured = {}, guidance = null, kitItems = [], userText = '') {
    ...
    // An escalation must change the actual response, not only the assessment.
    // Otherwise a newly urgent burn can still receive the old minor-care text.
    if (structured?.decision?.urgency === 'urgent') {
        const urgent = buildUrgentMedicalFallback(understanding, structured, guidance);
        if (urgent) return urgent;
    }

    const focused = buildFocusedFollowupReply(understanding, structured, userText);
    ...
    const topicLabels = {
        ...
        injury: 'injury', sprain: 'sprain', fracture: 'suspected fracture', dislocation: 'suspected dislocation', cpr: 'CPR', sleepiness: 'sleepiness', fatigue: 'fatigue',
    ...
    for (const action of (guidance.actions || [])) {
        let rendered = action;
        if (topics[0] === 'minor_wound' && structured?.relevantContext?.facts?.bleeding_status === 'not bleeding') {
            rendered = rendered.replace(/If it is bleeding, apply steady direct pressure with clean gauze or a clean cloth\.\s*/i, '');
            rendered = rendered.replace(/Once bleeding is controlled, /i, '');
        }
        lines.push(`- ${rendered}`);
    }
    ...
    const directKitByTopic = {
        ...
        cpr: ['CPR Face Shield / Barrier Device']
    };
    ...
    lines.push('');
    if (!(topics[0] === 'minor_wound' && structured?.relevantContext?.facts?.bleeding_status === 'not bleeding')) lines.push(guidance.summary);
...
function buildGeneralMedicalFallback(understanding, structured = {}) {
    ...
    const labels = {
        ...
        minor_wound: 'cut or wound', burn: 'burn', injury: 'injury', sprain: 'sprain', fracture: 'suspected fracture', dislocation: 'suspected dislocation', cpr: 'CPR', sleepiness: 'sleepiness',
...
function sameSituationUpdateLooksPlausible(text, topics, facts = {}) {
    ...
    const patterns = [
        ...
        /\b(vomit|vomiting|nauseous|nausea|dizzy|dizziness|faint|fainted|confused|nahihilo|nasusuka|nahimatay|nalilito|other symptoms|no other symptoms|none)\b/i,
        /\b(studying|study|barely drank|drank little|not enough water|dehydrated|hydration|water)\b/i,
    ];
...
function extractContextualFactUpdates(text, activeTopics = []) {
    ...
    if (topics.has('minor_wound') || topics.has('wound')) {
        const normalizedLocation = extractWoundLocation(value);
        if (normalizedLocation) updates.wound_location = normalizedLocation;
        ...
        for (const re of locationPatterns) {
            ...
            if (match) {
                ...
                if (!updates.wound_location) updates.wound_location = `${side}${match[2]}`.trim();
                break;
            }
        }
    }
    ...
    if (topics.has('burn')) {
        const loc = /\b(?:on|at|in)\s+(?:my|the)?\s*(right|left)?\s*(hand|arm|forearm|leg|face|chest|back|finger|kamay|braso|binti|mukha|dibdib|likod)\b/i.exec(n);
        if (loc) updates.burn_extent_or_location = `${loc[1] ? loc[1] + ' ' : ''}${loc[2]}`.trim();
    }
    if (/\b(pain|painful|hurt|hurts|sore)\b/i.test(n)) updates.pain_severity_or_red_flags = value;
    if (/\b(red|redness)\b/i.test(n)) updates.associated_symptoms_or_severity = value;
...
function isContextualFollowup(text, activeContext) {
    ...
    if (/^(what should i do|what do i do|what should i do now|what should i do right now|what do i do now|what do i do right now|what can i do|what now|what next)\??$/i.test(n)) return true;

    // Keep emergency operational updates attached to the active situation.
    if (/\b(not breathing|not breathing normally|only gasping|unresponsive|not responding|isn\'t responding|is not responding|aren\'t responding|are not responding|not responsive|collapsed|aed|defibrillator|call 911|called 911|dispatcher|emergency services)\b/i.test(n)) return true;

    // Natural symptom/detail updates can refer to the active situation without
    // saying 'it' or naming the injury again: 'it hurts and it's red',
    // 'still a little blood', 'it's swollen', 'there's a blister forming', etc.
    if (/\b(pain|painful|hurt|hurts|sore|red|redness|swollen|swelling|bleeding|blood|blister|blistering|skin|damaged|a little|slight|mild|moderate|severe|getting worse|worse|improving|better|crack|cracked|deformed|deformity)\b/i.test(n)) {
        const topics = new Set(activeContext.activeSituation.topics || []);
        if ([...topics].some(t => ['burn','minor_wound','injury','headache','dizziness','nausea','weakness','fever','skin_rash','eye_issue'].includes(t))) return true;
    }
    ...
    if (canBePendingAnswer && !introducesNewMedicalTopic && priorContext?.activeIntent === 'medical' && priorContext.pendingInfo?.length) {
        ...
        for (const category of priorContext.pendingInfo) {
            if (explicitlyAnswersForContext(lastUserText, category) || pendingAnswerLooksPlausible(lastUserText, category)) {
                pendingFacts[category] = category === 'wound_location' ? (extractWoundLocation(lastUserText) || String(lastUserText || '').trim()) : String(lastUserText || '').trim();
            }
        }
        ...
        if (sameSituationUpdateLooksPlausible(lastUserText, [...activeTopics], priorContext?.facts || {})) {
            ...
            if (activeTopics.has('minor_wound') && /\b(bleed|bleeding|blood|stopped|not anymore|no more|tumigil|wala na|dumudugo|dugo|malakas|kaunti|konti|marami)\b/i.test(t)) {
                ...
                pendingFacts.burn_extent_or_location = (/(?:on|at|in)\s+(?:my|the)?\s*(right|left)?\s*(hand|arm|forearm|leg|face|chest|back|finger|kamay|braso|binti|mukha|dibdib|likod)\b/i.exec(t) || [])[0] || t;
                ...
                pendingFacts.wound_location = extractWoundLocation(t) || t;
            }
        }
        const effectiveText = [
            priorContext.originMessage,
...
function buildMinorWoundReply(userText, structured = {}, contents = []) {
    ...
    if (latestBleeding) {
        return 'For a cut with a little bleeding, use **sterile gauze pads** from the kit and apply gentle pressure until the bleeding slows or stops. Then gently rinse the cut with clean water and cover it with an **adhesive bandage** if it is small.';
    }
...
function buildEmergencyFirstAidReply(userText, understanding = {}) {
    const text = String(userText || '').trim();
    const topics = new Set(understanding?.topics || []);
    const unresponsive = /unresponsive|not responding|isn'?t responding|is not responding|aren'?t responding|are not responding|not responsive|walang malay|unconscious|passed out/i.test(text);
    ...
    const cprEmergency = topics.has('cpr') || /not breathing normally|not breathing|only gasping|unresponsive|not responding/i.test(text);
    if (unresponsive || cprEmergency) return 'A person who has suddenly collapsed and is not responding needs emergency help right away. **Call 911 now.**\n\nWhile help is coming:\n1. **Check whether they are breathing normally.** If they are not breathing normally or are only gasping, start CPR immediately.\n2. **Place the heel of one hand in the center of the chest, put the other hand on top, and push hard and fast at 100–120 compressions per minute.** Let the chest come back up after each push.\n3. **Get an AED** if one is available and turn it on as soon as it arrives. Follow its voice prompts.\n4. Keep CPR going until the person starts breathing or moving normally, or trained responders take over.\n\n**Alert a teacher, school clinic staff, or another responsible adult immediately.** If the kit has its CPR face shield/barrier, it can be used according to its instructions during rescue breathing.\n\nIf you need any assistance with how to use an item from the kit, just ask.';
...
function buildNaturalClarificationReply(category, text, options = {}) {
    ...
    if (category === 'bleeding_status') {
        return filipino
            ...
            : `For a cut, you can gently rinse it with clean running water and cover it afterward. If it is bleeding, you can use **sterile gauze pads** from the kit and apply gentle pressure.\n\n${question}`;
    }
...
function isMedkitInventoryRequest(text) {
    const n = String(text || '').toLowerCase().replace(/[^a-z0-9\s]/g, ' ').replace(/\s+/g, ' ').trim();
    if (!/\b(med\s*kit|medkit|medical kit|medical aid kit|first[- ]aid kit|aid kit|kit)\b/i.test(n)) return false;
...
function buildContextualKitReply(text, contents = [], structured = {}, sessionMemory = null) {
    const n = String(text || '').toLowerCase().replace(/[’‘]/g, "'").replace(/[^a-z0-9\s']/g, ' ').replace(/\s+/g, ' ').trim();
    const asksKitForSituation = /\b(what (?:can|should) i (?:use|put)|what can i use|what should i use|what do i use|what can i put|what should i put|what do i need(?: from| for)?|do i need anything(?: from| in)?|is there anything(?: from| in)?|can i use (?:something|anything)?(?: from| in)?|which (?:item|thing|part) (?:can|should) i use)\b/i.test(n)
        || (/\b(kit|med kit|medkit|medical kit|first aid kit|aid kit)\b/i.test(n) && /\b(what|which|anything|something|need|can i|should i)\b/i.test(n) && /\b(use|put|apply|need|have|take)\b/i.test(n));
    const asksHowToUse = /\b(how do i use it|how do i use that|how can i use it|how should i use it|how is it used|how do i use this)\b/i.test(n);
    const activeContext = sessionMemory?.activeContext || structured?.relevantContext || structured?.context || {};
    const activeTopics = new Set(activeContext?.activeSituation?.topics || []);
    const facts = activeContext?.facts || {};
    const bleedingState = contextFactForDecision('bleeding_status', facts.bleeding_status || '');

    if (asksKitForSituation) {
        // Never let a kit question bypass an emergency response. The emergency
        // branch below must remain responsible for urgent action; the kit can
        // be discussed after the immediate emergency instructions are given.
        if (structured?.decision?.urgency === 'emergency' || structured?.understanding?.severity === 'emergency') return null;

        const topicPreferences = [
            ['minor_wound', ['sterile-gauze-pads', 'adhesive-bandages', 'medical-adhesive-tape']],
            ['burn', ['burn-dressing']],
            ['sprain', ['instant-cold-pack', 'elastic-bandage']],
            ['injury', ['instant-cold-pack', 'elastic-bandage']],
            ['fever', ['digital-thermometer']],
            ['eye_issue', ['sterile-saline-solution']],
            ['nosebleed', ['sterile-gauze-pads']],
            ['severe_bleeding', ['sterile-gauze-pads']],
            ['cold_exposure', ['emergency-thermal-blanket']],
        ];
        const preferredIds = [];
        for (const [topic, ids] of topicPreferences) {
            if (activeTopics.has(topic)) preferredIds.push(...ids);
        }

        // Use the physical kit reference directly for an active-context kit
        // question. The current turn may be a plain question such as "what can
        // I use from the kit?" and therefore have no medical decision of its
        // own; relying on that fresh decision made valid follow-ups return no
        // items.
        const byId = new Map(KIT.map(item => [item.id, item]));
        const selected = [];
        for (const id of preferredIds) {
            if (byId.has(id) && !selected.some(item => item.id === id)) selected.push(byId.get(id));
        }
        if (!selected.length && activeTopics.size) {
            const directMatches = KIT.filter(item => (item.topics || []).some(topic => activeTopics.has(topic)));
        ...
        if (activeTopics.has('minor_wound') && bleedingState !== 'still bleeding') {
            const bandage = byId.get('adhesive-bandages');
            if (bandage) {
                const how = buildSingleKitItemReply('how do i use adhesive bandage');
                const lines = ['For this cut, the main item you need from the kit is:', `\n**${bandage.name}** — ${bandage.uses[0]}.`];
                if (how) {
                    const steps = how.split('\n').filter(x => /^\d+\.\s*/.test(x)).slice(0, 3);
                    if (steps.length) lines.push('How to use it: ' + steps.map(x => x.replace(/^\d+\.\s*/, '')).join(' '));
                }
                lines.push('\nIf the cut starts bleeding again, **Sterile Gauze Pads** can be used to apply direct pressure.');
                return lines.join('\n');
            }
        }

        if (selected.length) {
            const lines = ['For this situation, the most relevant item' + (selected.length > 1 ? 's' : '') + ' from the kit ' + (selected.length > 1 ? 'are' : 'is') + ':'];
            for (const item of selected.slice(0, 3)) {
                const how = buildSingleKitItemReply('how do i use ' + item.name.toLowerCase());
                lines.push(`\n**${item.name}** — ${item.uses[0]}.`);
                if (how) {
                    const steps = how.split('\n').filter(x => /^\d+\.\s*/.test(x)).slice(0, 3);
                    if (steps.length) lines.push('How to use it: ' + steps.map(x => x.replace(/^\d+\.\s*/, '')).join(' '));
                }
            }
            lines.push('\nUse only the items that match the situation; you do not need the whole kit.');
            return lines.join('\n');
        }

        // A medical context may be known even when the current decision does
        // not map a kit item. Do not dump the inventory; explain that no
        // specific item is established instead.
        if (activeTopics.size) return 'For this situation, no specific kit item is needed based on the information so far. If you tell me what you want to use the kit for, I can narrow it down.';
    }

    if (asksHowToUse) {
        const prior = [...(contents || [])].reverse().find(c => c?.role === 'model')?.parts?.map(p => p?.text || '').join(' ') || '';
        if (activeTopics.has('minor_wound') && bleedingState === 'still bleeding') {
            return buildSingleKitItemReply('how do i use sterile gauze') || 'For the cut, place **Sterile Gauze Pads** over the wound and apply steady direct pressure until the bleeding is controlled.';
        }
        const candidates = [
            ['adhesive bandage', 'Adhesive Bandages (Assorted Sizes)'],
            ['sterile gauze', 'Sterile Gauze Pads'],
            ['medical adhesive tape', 'Medical Adhesive Tape'],
            ['burn dressing', 'Burn Dressing (Sterile Non-Stick)'],
            ['antiseptic', 'Antiseptic Solution / Wipes'],
            ['elastic bandage', 'Elastic Bandage (ACE Type)'],
            ['triangular bandage', 'Triangular Bandage'],
        ...
        for (const [needle, name] of candidates) {
            if (new RegExp('\\b' + needle.replace(/[.*+?^${}()|[\\]\\]/g, '\\\\$&') + '\\b', 'i').test(prior)) {
                return buildSingleKitItemReply('how do i use ' + needle) || `**${name}** is used according to the kit reference and product instructions.`;
            }
        }
        if (activeTopics.has('minor_wound')) {
            return bleedingState === 'still bleeding'
                ? 'For the cut, place **Sterile Gauze Pads** over the wound and apply steady direct pressure until the bleeding is controlled. Then rinse the cut and cover it with an **Adhesive Bandage** if it is small.'
                : 'For the cut, gently rinse it with clean running water, then place a clean **Adhesive Bandage** over it. If it starts bleeding, use **Sterile Gauze Pads** and apply steady pressure.';
        }
    }
    return null;
}
...
function getRagKnowledge({ userText, understanding, decision, context }) {
    const topic = Array.isArray(understanding?.topics) ? understanding.topics.join(' ') : '';
    const facts = Object.entries(context?.facts || {}).map(([k, v]) => `${k} ${v}`).join(' ');
    const urgency = decision?.urgency || '';
    const query = [userText, topic, urgency, facts].filter(Boolean).join(' ');
    try {
        return rag.topChunks(query, 4);
    } catch {
        // Knowledge retrieval is supplementary. A retrieval failure must never
        // block the deterministic safety path or the communication layer.
        return [];
    }
}
...
async function handleChat(req, res) {
    ...
    const { contents, session_memory } = body;
    ...
    const singleKitItemReply = buildSingleKitItemReply(lastUserText);
    if (singleKitItemReply) return respond(singleKitItemReply, assessment());
    // A contextual "what can I use from the kit?" question should answer the
    // active situation, not dump the entire inventory. Likewise, "how do I
    // use it?" should continue the previously recommended item when one is
    // identifiable from the immediately preceding assistant reply.
    const contextualKitReply = buildContextualKitReply(lastUserText, contents, structured, sessionMemory);
    if (contextualKitReply) {
        const kitAwareState = buildSituationState(structured, {
            kitRecommendation: /Sterile Gauze/i.test(contextualKitReply)
                ? 'Sterile Gauze Pads: bleeding control; Adhesive Bandages: cover a small wound after bleeding is controlled.'
                : 'Adhesive Bandages (Assorted Sizes): cover a small cleaned wound.'
        });
        return respond(contextualKitReply, kitAwareState);
    }
    ...
    if (structured.understanding.messageType === 'medical') {
        ...
        if (structured.decision.clarificationOnly) {
            ...
            const fallback = buildGeneralMedicalFallback(structured.understanding, {
                ...structured,
                decision: { ...structured.decision, missingInfo: [firstMissing] }
            });
            return respond(ai.reply || fallback, situationState);
        }
        ...
        // AI is the communication layer, not the medical authority. The
        // deterministic pipeline has already decided the safe topic, urgency,
        // facts, guidance, and kit relevance. Qwen uses that structured state
        // to construct the context-aware conversational answer. If Ollama is
        // unavailable, the deterministic formatter remains the safe fallback.
        const ragMatches = getRagKnowledge({
            userText: lastUserText,
            understanding: structured.understanding,
            decision: structured.decision,
        ...
        const formatted = ai.reply || buildFormattedMedicalReply(
...
async function handleAdmin(req, res, url) {
    if (process.env.VERCEL && (req.method === 'POST' || req.method === 'DELETE')) {
        return sendJson(res, 503, { error: 'Persistent RAG document storage is not configured for this Vercel deployment. Built-in references remain available; manage uploaded documents on the local kiosk server.' });
    }
