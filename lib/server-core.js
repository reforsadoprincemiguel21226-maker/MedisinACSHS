...
function isContextualFollowup(text, activeContext) {
    ...
    // 'still a little blood', 'it's swollen', etc.
    if (/\b(pain|painful|hurt|hurts|sore|red|redness|swollen|swelling|bleeding|blood|a little|slight|mild|moderate|severe|getting worse|worse|improving|better)\b/i.test(n)) {
...
function buildEmergencyFirstAidReply(userText, understanding = {}) {
    const text = String(userText || '').trim();
    const topics = new Set(understanding?.topics || []);
    const unresponsive = /unresponsive|not responding|isn'?t responding|is not responding|collapsed|walang malay|unconscious|passed out/i.test(text);
...
function buildContextualKitReply(text, contents = [], structured = {}, sessionMemory = null) {
    ...
    const asksKitForSituation = /\b(what (?:can|should) i (?:use|put)|what can i use|what should i use|what do i use|what can i put|what should i put|what (?:in|from) the (?:kit|med kit|medkit|medical kit|first aid kit|aid kit) can i use|what can i use from the (?:kit|med kit|medkit|medical kit|first aid kit|aid kit))\b/i.test(n)
        || (/\b(kit|med kit|medkit|medical kit|first aid kit|aid kit)\b/i.test(n) && /\b(what|which)\b/i.test(n) && /\b(use|put|apply)\b/i.test(n));
    ...
    if (asksKitForSituation && activeTopics.has('minor_wound')) {
        if (bleedingState === 'still bleeding') {
            return 'For the cut, use **Sterile Gauze Pads** to apply steady direct pressure until the bleeding is controlled. Once controlled, use an **Adhesive Bandage** to cover a small cut after cleaning. **Medical Adhesive Tape** can secure gauze if needed.';
        }
        return 'For the cut, use an **Adhesive Bandage (Assorted Sizes)** after gently rinsing it with clean running water. **Sterile Gauze Pads** are useful if it starts bleeding again, and **Medical Adhesive Tape** can secure gauze if needed.';
    }
