...
const TOPIC_BASELINE_SEVERITY = {
  ...
  injury: "minor",
  sprain: "minor",
  fracture: "potentially_severe",
  dislocation: "potentially_severe",
  cpr: "potentially_severe",
...
function deriveSeverityForTopic(normalizedText, topic) {
  ...
  if (topic === "cpr" && ["cardiac arrest", "heart stopped", "not breathing", "only gasping", "not responding", "unresponsive"].some(p => containsPhrase(normalizedText, p))) {
    return "emergency";
  }

  if ((topic === "fracture" || topic === "dislocation") && [
    "bone sticking out", "bone protruding", "bone is sticking out", "open fracture",
    "severe deformity", "major deformity", "cold hand", "cold foot", "numb hand", "numb foot",
    "no pulse", "cannot feel pulse", "uncontrolled bleeding"
  ].some(p => containsPhrase(normalizedText, p))) {
    return "emergency";
  }

  // Burn progression needs explicit escalation even when the user never
  // uses the word "severe". A growing blister or visibly damaged skin is new
  // evidence about the burn itself, especially when it appears in a follow-up
  // turn that is reconnected to the original burn context. Keep this at
  // potentially severe unless there is stronger emergency evidence.
  if (topic === "burn" && [
    "blister got bigger", "blister is getting bigger", "blister grew",
    "blisters got bigger", "blisters are getting bigger",
    "skin looks really damaged", "skin looks badly damaged",
    "skin is badly damaged", "skin looks damaged", "skin is damaged",
    "skin is peeling off", "skin peeled off", "skin is coming off",
    "large blister", "large blisters", "deep burn", "deeply burned"
  ].some((p) => containsPhrase(normalizedText, p))) {
    severity = "potentially_severe";
  }

  // Burns involving the hand warrant prompt medical assessment even when they
  // initially look minor. This is deliberately not an emergency by itself.
  if (topic === "burn" && /\b(?:burn(?:ed)?|burning)\b.*\b(?:hand|hands|finger|fingers|kamay|daliri)\b|\b(?:hand|hands|finger|fingers|kamay|daliri)\b.*\b(?:burn(?:ed)?|burning)\b/i.test(normalizedText)) {
    severity = maxSeverity(severity, "potentially_severe");
  }
  ...
  // minor.
  if ((topic === "minor_wound" || topic === "fracture" || topic === "dislocation") &&
...
function understandMessage(rawText) {
  ...
  const directEmergencyPhrases = [
    "unresponsive", "not responding", "isn't responding", "is not responding", "collapsed", "suddenly collapsed", "collapsed", "suddenly collapsed",
