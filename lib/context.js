...
function updateContext(previousContext, message, understanding, decision) {
  ...
  // A non-answer, non-medical turn is a topic switch. Suspend the active
  // situation instead of allowing it to leak into unrelated requests. The
  // conversation history still exists outside this compact active state, so
  // an explicit later reference can re-establish the prior topic.
  if (u.messageType === 'other') return blankContext();
  return context;
}
...
module.exports = { blankContext, updateContext, getRelevantContext, extractKnownLocation };
