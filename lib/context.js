...
function extractWoundLocation(text) {
  ...
  const patterns = [
    /\b(?:on|at|in|sa)\s+(?:my|the|aking)\s+(right|left)\s+(hand|palm|wrist|arm|forearm|elbow|shoulder|leg|thigh|calf|ankle|foot|knee|hip|head|face|finger|daliri|palad|kamay|pulso|braso|siko|balikat|binti|hita|bukong-bukong|paa|tuhod|balakang|mukha)\b/i,
    /\b(?:on|at|in|sa)\s+(?:my|the|aking)\s+(hand|palm|wrist|arm|forearm|elbow|shoulder|leg|thigh|calf|ankle|foot|knee|hip|head|face|finger|daliri|palad|kamay|pulso|braso|siko|balikat|binti|hita|bukong-bukong|paa|tuhod|balakang|mukha)\b/i,
    /\b(?:on|at|in|sa)\s+(?:my|the|aking)\s+(index|middle|ring|little|pinky|thumb)\s+(finger|daliri)\b/i,
    /\b(?:my|the|aking)\s+(right|left)\s+(hand|palm|wrist|arm|forearm|elbow|shoulder|leg|thigh|calf|ankle|foot|knee|hip|head|face|finger|daliri|palad|kamay|pulso|braso|siko|balikat|binti|hita|bukong-bukong|paa|tuhod|balakang|mukha)\b/i,
    /\b(?:my|the|aking)\s+(index|middle|ring|little|pinky|thumb)\s+(finger|daliri)\b/i,
    /\b(?:cut|wound|hiwa|sugat)\s+(?:is\s+)?(?:on|at|in|sa)\s+(?:my|the|aking)\s+(right|left)\s+(hand|palm|wrist|arm|forearm|elbow|shoulder|leg|thigh|calf|ankle|foot|knee|hip|head|face|finger|daliri|palad|kamay|pulso|braso|siko|balikat|binti|hita|bukong-bukong|paa|tuhod|balakang|mukha)\b/i,
    /\b(?:cut|wound|hiwa|sugat)\s+(?:is\s+)?(?:on|at|in|sa)\s+(?:my|the|aking)\s+(index|middle|ring|little|pinky|thumb)\s+(finger|daliri)\b/i,
    /\b(?:cut|wound|hiwa|sugat)\s+(?:my|the|aking)?\s*(finger|hand|palm|wrist|arm|forearm|elbow|shoulder|leg|thigh|calf|ankle|foot|knee|hip|head|face|daliri|palad|kamay|pulso|braso|siko|balikat|binti|hita|bukong-bukong|paa|tuhod|balakang|mukha)\b/i,
    /\bI\s+(?:cut|hurt)\s+(?:my|the)?\s*(finger|hand|palm|wrist|arm|forearm|elbow|shoulder|leg|thigh|calf|ankle|foot|knee|hip|head|face|daliri|palad|kamay|pulso|braso|siko|balikat|binti|hita|bukong-bukong|paa|tuhod|balakang|mukha)\b/i,
    /^(finger|hand|palm|wrist|arm|forearm|elbow|shoulder|leg|thigh|calf|ankle|foot|knee|hip|head|face|daliri|palad|kamay|pulso|braso|siko|balikat|binti|hita|bukong-bukong|paa|tuhod|balakang|mukha)$/i,
  ];
  for (const re of patterns) {
    const m = n.match(re);
    if (!m) continue;
    const a = m[1], b = m[2];
    if (b) return `${a} ${b}`.trim();
    return a;
  }
  return null;
}
...
function updateContext(previousContext, message, understanding, decision) {
  ...
  if (u.messageType === 'medical') {
    ...
    // Extract concrete wound location from the current message even when it
    // was part of the original complaint (for example, "I cut my palm while
    // opening a can"). Do not store the whole sentence as a location fact.
    if (u.topics?.includes('minor_wound')) {
      const location = extractWoundLocation(text);
      if (location) {
        context.facts.wound_location = location;
    ...
    for (const category of answerableCategories) {
      if (explicitlyAnswers(text, category)) {
        context.facts[category] = category === 'wound_location' ? (extractWoundLocation(text) || context.facts.wound_location || '') : text;
    ...
    if (u.topics?.includes('minor_wound') && explicitlyAnswers(text, 'wound_location')) {
      context.facts.wound_location = extractWoundLocation(text) || context.facts.wound_location || '';
      context.pendingInfo = context.pendingInfo.filter(x => x !== 'wound_location');
    }
    return context;
  }
...
module.exports = { blankContext, updateContext, getRelevantContext, extractKnownLocation, extractWoundLocation, sanitizeInputContext };
