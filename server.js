...
const { blankContext, updateContext, getRelevantContext } = require('./lib/context.js');
const { mapKitItems } = require('./lib/map-kit.js');
const { buildSituationState } = require('./lib/situation.js');
...
function buildFormattedMedicalReply(understanding, structured = {}, guidance = null, kitItems = [], userText = '') {
    ...
    const openers = {
        ...
        minor_wound: /finger|hand|kamay|daliri/i.test(text)
            ? 'A cut on the hand or finger can bleed and become irritated easily, so start by cleaning and protecting it.'
            : 'For a cut or wound, start with simple wound care and check whether there is active bleeding.',
...
function getStructuredMedicalState(lastUserText, contents) {
    const userItems = (contents || []).filter((item) => item?.role !== 'model');
    const priorItems = userItems.slice(0, -1);
    const priorContext = buildStructuredContext(priorItems);
...
function structuredMedicalGate(lastUserText, contents) {
    ...
    return getStructuredMedicalState(lastUserText, contents);
}
...
async function handleChat(req, res) {
    ...
    const structured = reconcileDecisionWithContext(structuredMedicalGate(lastUserText, contents));
    ...
    if (isCrisisMessage(lastUserText)) {
        return sendChatReply(res, buildEmotionalSupportReply(lastUserText, contents), assessment());
    }
    if (isMedicationRequest(lastUserText)) {
        return sendChatReply(res, buildMedicationBoundaryReply(), assessment());
    }
    if (isCapabilityRequest(lastUserText)) {
        return sendChatReply(res, buildCapabilityReply(), assessment());
    }
    if (isKitAvailabilityRequest(lastUserText)) {
        const reply = buildKitAvailabilityReply(lastUserText);
        if (reply) return sendChatReply(res, reply, assessment());
    }
    if (isMedkitExplanationRequest(lastUserText, contents)) {
        return sendChatReply(res, buildMedkitExplanationReply(), assessment());
    }
    if (isMedkitInventoryRequest(lastUserText)) {
        return sendChatReply(res, buildMedkitInventoryReply(), assessment());
    }
    ...
    if (isCasualMessage(lastUserText) && structured.understanding.messageType !== 'medical') {
        return sendChatReply(res, buildCasualReply(lastUserText), assessment());
    }
    ...
    if (hospitalRequested || locationReply || (suppliedLocation && hospitalContext) || explicitLocationHospitalRequest) {
        ...
        if (!locationKey) {
            ...
            return sendChatReply(res, 'What city or municipality are you currently in?', updated);
        }
        ...
        return sendChatReply(res, reply, hospitalAssessment);
    }
    ...
    if (structured.understanding.messageType === 'medical') {
        ...
        return sendChatReply(res, formatted, situationState);
    }
    ...
    if (/^(?:how does this work|how do these work|what is this)\??$/i.test(lastUserText.trim())) {
        return sendChatReply(res, 'If you mean the MedisinACSHS kiosk, it combines the first-aid kit with an assistant that can explain the kit, provide basic first-aid information, and help with emergency or hospital-related questions.', assessment());
    }
    return sendChatReply(res, 'I can help with basic first-aid and health information, the MedisinACSHS kit, emotional support, and hospital lookup by location. What would you like help with?', assessment());
}

function sendChatReply(res, reply, assessment) {
    return sendJson(res, 200, { reply, assessment });
}
...
if (require.main === module) {
    ...
    server.listen(PORT, () => {
        ...
        console.log(`Deterministic medical response path active; Ollama extension configured for ${OLLAMA_MODEL} at ${OLLAMA_URL}`);
        console.log(ADMIN_TOKEN ? `Admin panel at http://localhost:${PORT}/admin.html` : 'ADMIN_TOKEN not set — admin panel disabled');
    });
}

module.exports = { handleChat, handleAdmin, serveStatic, createLocalServer };
